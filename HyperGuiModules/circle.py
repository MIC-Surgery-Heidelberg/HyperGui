...
class Circle:
    def __init__(self, circle_frame, listener):
        self.root = circle_frame
    ...
    def __update_selected_data_cube(self, event):
        ...
        mask_path = os.path.dirname(dc_path) +'/'+self.listener.output_folder_hypergui +"/MASK_COORDINATES_CIRCULAR.csv"
    ...
    def _build_info_label(self):
        self.info_label = make_label_button(self.root, text='bp (circular)', command=self.__info, width=8)
    ...
    def _build_size_inputs(self):
        ...
        rad_text = make_text(subFrame, content="Diameter 1: ", bg=tkcolour_from_rgb(BACKGROUND), column=11, row=24,
                                   width=10)
        self.radius_1_entry = make_entry(subFrame, row=24, column=12, width=5, pady=(5, 5))
        self.radius_1_entry.insert(0, str(self.radius_1))
        self.radius_1_entry.bind("<Return>", self.__update_radius)
        
        rad_text = make_text(subFrame, content="Diameter 2: ", bg=tkcolour_from_rgb(BACKGROUND), column=11, row=25,
                                   width=10)
        self.radius_2_entry = make_entry(subFrame, row=25, column=12, width=5, pady=(5, 5))
        self.radius_2_entry.insert(0, str(self.radius_2))
        self.radius_2_entry.bind("<Return>", self.__update_radius)
        
        rad_text = make_text(subFrame, content="Angulation: ", bg=tkcolour_from_rgb(BACKGROUND), column=11, row=26,
                                   width=10)
        self.rotation_entry = make_entry(subFrame, row=26, column=12, width=5, pady=(5, 5))
        self.rotation_entry.insert(0, str(int(round(self.rotation/0.01745))))
        self.rotation_entry.bind("<Return>", self.__update_radius)
    ...
    def _draw_points(self):
        if self.original_image_data is not None:
            ...
            for point in not_none:
                ...
                self._draw_a_circle(not_none[idx], self.radius_1, self.radius_2, self.rotation, copy_data, color = self.color)
    ...
    @staticmethod
    def _draw_a_circle(point, radius1, radius2, rotation, image, color = (113,255,66)):
        r0, c0 = point
        rr, cc = ellipse_perimeter(int(c0), int(r0), radius1, radius2, orientation = rotation)
        for i in range(len(rr)):
            image[rr[i] % 480, cc[i] % 640] = (int(color[0]), int(color[1]), int(color[2]))
        return image
    ...
    def __save_coords(self, save_tif = False):
        ...
        output_path2 = path + "/"+self.listener.output_folder_hypergui+"/MASK_COORDINATES_CIRCULAR" + '.csv'
        if os.path.exists(output_path1) or os.path.exists(output_path2):
            self.unfocus = True
            yn = messagebox.askquestion ('Verify','Are you sure you want to override mask.csv and MASK_CORDINATES_CIRCULAR.csv?',icon = 'warning')
            if yn == "yes":
                self.unfocus = False
                self.__save_points()
                self.__save_mask()
    ...
    def __get_coords_list(self):
        ...
        data = [[float(point_coords[i][0] + 1), float(point_coords[i][1] + 1)] for i in range(0, 1000000) if
    ...
    def __save_mask(self):
        ...
        mask_array = np.zeros((480, 640))
        for point in points:
            r0, c0 = point
            rr, cc = ellipse(c0, r0, self.radius_1, self.radius_2, rotation = -self.rotation)
            for i in range(len(rr)):
                mask_array[rr[i] % 480, cc[i] % 640] = 1
        path = os.path.dirname(self.get_selected_data_cube_path())
        ...
        with open(path + '/'+self.listener.output_folder_hypergui + "/_circular" + '.txt', 'w') as fp:
            pass
        
    def __save_points(self):
        data = self.__get_coords_list()
        data = [[float(self.radius_1),  float(self.radius_2)]] + [[float(self.rotation), 0]]+ data
        if len(data)>0:
            path = os.path.dirname(self.get_selected_data_cube_path())
            if not os.path.exists(path + '/'+self.listener.output_folder_hypergui):
                os.mkdir(path + '/'+self.listener.output_folder_hypergui)
            output_path = path + '/'+self.listener.output_folder_hypergui + "/MASK_COORDINATES_CIRCULAR" + '.csv'
    ...
    def __load_mask(self, path):
        ...
        self.radius_1 = int(coords[0][0])
        self.radius_2 = int(coords[0][1])
    ...
    def __update_radius(self, event = None):
        self.radius_2 = int(int(self.radius_1_entry.get())/2)
        self.radius_1 = int(int(self.radius_2_entry.get())/2)
